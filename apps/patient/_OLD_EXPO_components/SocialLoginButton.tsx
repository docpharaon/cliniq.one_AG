...
import { colors, spacing, radius, typography } from '@cliniqone/ui';
...
export function SocialLoginButton({
    ...
    const defaultLabel = isApple ? 'Sign in with Apple' : 'Sign in with Google';
    const displayLabel = label || defaultLabel;

    // Apple HIG: Apple button must be black or white, with the Apple logo
    // Google: White/light background with colored G logo
    const buttonStyle = isApple
        ? [styles.button, styles.appleButton, disabled && styles.disabled]
        : [styles.button, styles.googleButton, disabled && styles.disabled];

    const labelStyle = isApple ? styles.appleLabel : styles.googleLabel;
    ...
    return (
        ...
        >
            {loading ? (
                <ActivityIndicator
                    size="small"
                    color={isApple ? '#FFFFFF' : colors.textPrimary}
                />
            ) : (
