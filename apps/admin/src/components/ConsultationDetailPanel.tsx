...
import {
    ...
    Printer, Download, User, Stethoscope, FileText, Activity,
...
export default function ConsultationDetailPanel({ consultation: c, onClose, onUpdated, adminUserId }: Props) {
    ...
    function handlePrintPDF() {
        ...
        const reportContent = c.report
            ? JSON.stringify(c.report, null, 2)
            : 'No report available';
        const prescriptionContent = c.prescription
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>Consultation ${c.id.slice(0, 8)} — cliniq.one</title>
                <style>
                    body { font-family: 'Segoe UI', Tahoma, sans-serif; padding: 40px; color: #1a1a2e; max-width: 800px; margin: auto; }
                    h1 { font-size: 22px; border-bottom: 2px solid #2dd4bf; padding-bottom: 8px; }
                    h2 { font-size: 16px; color: #2dd4bf; margin-top: 24px; }
                    .meta { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin: 16px 0; }
                    .meta-item { background: #f8f9fa; padding: 8px 12px; border-radius: 6px; }
                    .meta-label { font-size: 11px; color: #666; text-transform: uppercase; }
                    .meta-value { font-size: 14px; font-weight: 600; }
                    pre { background: #f8f9fa; padding: 16px; border-radius: 8px; font-size: 12px; white-space: pre-wrap; }
                    .footer { margin-top: 40px; border-top: 1px solid #eee; padding-top: 16px; font-size: 11px; color: #999; }
            ...
            <body>
                ...
                <div class="meta">
                    <div class="meta-item"><div class="meta-label">Case ID</div><div class="meta-value">${c.id}</div></div>
                    <div class="meta-item"><div class="meta-label">Status</div><div class="meta-value">${st.label}</div></div>
                    <div class="meta-item"><div class="meta-label">Patient</div><div class="meta-value">${c.patient_name}</div></div>
                    <div class="meta-item"><div class="meta-label">Doctor</div><div class="meta-value">${c.doctor_name ?? 'Unassigned'}</div></div>
                    <div class="meta-item"><div class="meta-label">Specialty</div><div class="meta-value">${c.specialty?.replace('_', ' ')}</div></div>
                    <div class="meta-item"><div class="meta-label">Priority</div><div class="meta-value">${c.priority}${c.urgent_fee ? ` (+${c.urgent_fee} tokens)` : ''}</div></div>
                    <div class="meta-item"><div class="meta-label">Created</div><div class="meta-value">${formatDate(c.created_at)}</div></div>
                    <div class="meta-item"><div class="meta-label">Concluded</div><div class="meta-value">${formatDate(c.concluded_at)}</div></div>
                </div>
                ...
                <div class="footer">
                    Generated by cliniq.one Admin Panel — ${new Date().toLocaleString()}<br/>
                    This document is for medicolegal purposes. Store securely offline.
                </div>
            </body>
            </html>
        `);
                    ...
                    {isConcluded && !isPurged && (
                        <div className="flex gap-2">
                            <button
                                ...
                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-accent text-bg-primary text-sm font-semibold hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(45,212,191,0.4)] transition-all"
                            >
                            ...
                            <button
                                onClick={handlePrintPDF}
                                className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-border text-sm text-accent hover:bg-accent-faded transition-colors"
                            >
